...
//   shouldn't be dosed against without a glance at the primary app. (Note: local dev in BST
//   inflates this by 1h via the documented FactoryTimestamp parse quirk; production on
//   Render/UTC reads correctly.)
function dosingContext(data) {
  const gc = getGlucoseCache();
  const current = (gc && !gc.error) ? gc.value : null;
  let ageMin = null;
  if (current != null) {
    const ts = new Date(gc.timestamp).getTime() || gc.fetchedAt;
...
async function forecastHypoRisk() {
  ...
  let projected = ctx.effective;
  let calibrated = factorUsable;
  if (factorUsable) {
    projected -= iobActing * factor;
    if (data.settings.carbRatio) projected += cobAbsorbing * (factor / data.settings.carbRatio);
    else if (cobAbsorbing > 10) calibrated = false; // carbs will help, but can't be quantified
  }

  if (ctx.projected != null && ctx.projected < ctx.current) factors.push('glucose is trending down');
  if (iobActing > 0.3) {
  ...
  // Post-workout drop window still open: the un-elapsed share of that type's median drop.
  const profiles = workoutTypeProfiles(data);
  const now = Date.now();
  for (const w of data.activities.filter(a => a.type === 'workout' && a.endTime)) {
    const hoursSince = (now - new Date(w.endTime).getTime()) / 3600000;
    ...
    const p = profiles.find(x => x.type === (w.workoutType || 'Exercise'));
    if (!p || p.dropShare < 0.6 || p.medianDrop < 1.5) continue;
    const remaining = p.medianDrop * Math.max(0, 1 - hoursSince / Math.max(0.5, p.medianHoursToNadir));
    if (remaining > 0.3) {
      projected -= remaining;
      factors.push(`inside "${p.type}"'s drop window — ~${remaining.toFixed(1)} mmol/L of its usual fall may still be coming`);
    }
  }
...
module.exports = {
  ...
  forecastHypoRisk, simulateActivity,
};
