...
// ─── Carb absorption ──────────────────────────────────────────────────
// Absorption used to be flat-linear over 180min for every meal, which sat badly against the
// exponential insulin curve: insulin peaks at 75min while linear carbs trickle in at a constant
// rate, so the simulation showed an exaggerated mid-window dip after any large meal.
//
// Rate is now triangular - ramps up to a peak, then tails to zero by `duration` - which is the
// standard pragmatic shape and integrates to exactly 1 (all carbs eventually absorbed). Meals
// logged from a preset get their OWN peak/duration derived from how they've actually behaved
// (see buildCarbProfiles), so a coffee and a pizza no longer absorb identically.
const CARB_DEFAULT_PEAK_MIN = 60;
const CARB_DEFAULT_DURATION_MIN = 180;
const clamp = (v, lo, hi) => Math.min(hi, Math.max(lo, v));

// Fraction of the meal absorbed by t minutes, under the triangular rate profile.
function carbAbsorbedFraction(t, peakMin, durationMin) {
  if (t <= 0) return 0;
  if (t >= durationMin) return 1;
  const peak = clamp(peakMin, 5, durationMin - 5);
  const h = 2 / durationMin;                     // rate peak, so the triangle's area is 1
  if (t <= peak) return h * t * t / (2 * peak);  // rising limb
  const tail = durationMin - peak;               // falling limb
  return h * peak / 2 + h * (tail * tail - (durationMin - t) ** 2) / (2 * tail);
}
function cobFraction(t, peakMin = CARB_DEFAULT_PEAK_MIN, durationMin = CARB_DEFAULT_DURATION_MIN) {
  return 1 - carbAbsorbedFraction(t, peakMin, durationMin);
}

// Per-meal absorption shape from meal memory. NOTE the proxy: what we measure is time-to-peak
// GLUCOSE, which lands later than peak absorption (glucose peaks where carb absorption and
// insulin action cross), so it's scaled rather than used directly - and clamped, so a single odd
// meal can't produce an absurd curve. Meals flagged delayed-rise (the fat/protein signature) get
// a long tail, which is precisely the case flat-linear handled worst.
function buildCarbProfiles(data) {
  const profiles = new Map();
  const byName = {};
  for (const b of data.boluses) {
    if (!b.mealName || !(b.carbs > 0)) continue;
    const k = b.mealName.toLowerCase();
    (byName[k] = byName[k] || []).push(b);
  }
  for (const [name, logs] of Object.entries(byName)) {
    const outcomes = mealOutcomes(data, logs);
    if (outcomes.length < 2) continue;
    const peakGlucoseMin = median(outcomes.map(o => o.timeToPeakMin));
    if (!(peakGlucoseMin > 0)) continue;
    const checked = outcomes.filter(o => o.delayedRise !== null);
    const delayedShare = checked.length ? outcomes.filter(o => o.delayedRise).length / checked.length : 0;
    let durationMin = clamp(peakGlucoseMin * 3, 120, 360);
    if (delayedShare >= 0.5) durationMin = Math.max(durationMin, 300);
    profiles.set(name, {
      peakMin: clamp(peakGlucoseMin * 0.7, 20, 150),
      durationMin,
      sessions: outcomes.length,
      delayed: delayedShare >= 0.5,
    });
  }
  return profiles;
}
function carbParamsFor(b, profiles) {
  const p = (profiles && b.mealName) ? profiles.get(b.mealName.toLowerCase()) : null;
  return p || { peakMin: CARB_DEFAULT_PEAK_MIN, durationMin: CARB_DEFAULT_DURATION_MIN };
}

// Carb absorption expected within the next `horizonMin` minutes.
function carbAbsorptionWithin(data, horizonMin, at = Date.now(), profiles = null) {
  let grams = 0;
  for (const b of data.boluses) {
    if (!(b.carbs > 0)) continue;
    const age = (at - b.time) / 60000;
    const { peakMin, durationMin } = carbParamsFor(b, profiles);
    if (age < 0 || age >= durationMin) continue;
    grams += b.carbs * (cobFraction(age, peakMin, durationMin) - cobFraction(age + horizonMin, peakMin, durationMin));
  }
  return grams;
}
function carbsOnBoard(data, at = Date.now(), profiles = null) {
  return data.boluses.reduce((s, b) => {
    if (!(b.carbs > 0)) return s;
    const { peakMin, durationMin } = carbParamsFor(b, profiles);
    return s + b.carbs * cobFraction((at - b.time) / 60000, peakMin, durationMin);
  }, 0);
}
...
function simulateForward(data, { start, factor, carbRatio, horizonMin, residualRate, drags, profiles }) {
  const now = Date.now();
  ...
  for (let off = 0; off < horizonMin; off += SIM_STEP_MIN) {
    ...
    if (factor) {
      ...
      if (carbToGlucose) dG += carbAbsorptionWithin(data, SIM_STEP_MIN, at, profiles) * carbToGlucose;
    }
...
async function forecastHypoRisk() {
  ...
  const carbRatio = data.settings.carbRatio;
  // Per-meal absorption shapes, built once and reused across every simulation step.
  const profiles = buildCarbProfiles(data);
  const iobActing = insulinActionWithin(data, HORIZON_MIN);
  const cobAbsorbing = carbAbsorptionWithin(data, HORIZON_MIN, Date.now(), profiles);
  const iobNow = parseFloat(activeInsulin(data).toFixed(1));
  const cobNow = Math.round(carbsOnBoard(data, Date.now(), profiles));
  const factors = [];
  ...
  if (observedRate != null) {
    ...
    if (factor) {
      modelRateNow = (-insulinActionWithin(data, SIM_STEP_MIN) * factor
        + (carbRatio ? carbAbsorptionWithin(data, SIM_STEP_MIN, Date.now(), profiles) * (factor / carbRatio) : 0)) / SIM_STEP_MIN;
    }
  ...
  const sim = simulateForward(data, {
    start: ctx.current, factor, carbRatio, horizonMin: HORIZON_MIN, residualRate, drags, profiles,
  });
...
async function simulateActivity(type, minutes, intensity) {
  ...
  const cobEffect = (factorUsable && data.settings.carbRatio)
    ? carbAbsorptionWithin(data, windowMin, Date.now(), buildCarbProfiles(data)) * (factor / data.settings.carbRatio) : 0;
