...
const { fetchGlucose, getGlucoseCache } = require('./libre');
const {
  analysePatterns, suggestMealDose, resolveEntryTime, glucoseAt,
  getDailySummary, checkInsulinHealth,
} = require('./analysis');
...
function registerRoutes(app) {
  ...
  app.post('/api/login', (req, res) => {
    if (req.body.password === APP_PASSWORD) {
      res.cookie('auth', AUTH_TOKEN, { maxAge: 365*24*60*60*1000, httpOnly: true, sameSite: 'lax' });
  ...
  app.get('/api/meal-suggestion', requireAuth, async (req, res) => {
    ...
    try { res.json(await suggestMealDose(carbs)); }
