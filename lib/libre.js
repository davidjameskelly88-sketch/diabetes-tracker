...
// FactoryTimestamp is UTC but arrives as a string with NO timezone designator (e.g.
// "7/22/2026 9:20:42 AM"), so new Date() parses it in whatever local zone the runtime is in -
// server OR browser. That's harmless for a *difference* of two such times (the delta chip,
// computeTrend: the offset cancels), but for the reading's AGE it shifts every fresh reading by
// the local UTC offset (an hour during BST), which made the "sensor may be changing" warning
// fire constantly on a UK phone. Parse the M/D/YYYY h:mm:ss AM/PM components explicitly as UTC
// so the epoch is correct regardless of the parsing machine's timezone. Falls back to Date.parse
// for any unexpected format.
function parseLibreTimestamp(s) {
  const m = /^(\d{1,2})\/(\d{1,2})\/(\d{4})[ T]+(\d{1,2}):(\d{2}):(\d{2})\s*(AM|PM)?$/i.exec(String(s).trim());
  if (!m) { const t = new Date(s).getTime(); return isNaN(t) ? null : t; }
  let h = +m[4];
...
// LibreLinkUp's follower graph endpoint returns ~12h of ~15-min-spaced history for a
// connection. We use it only to *seed* a sparse history (cold start, redeploy, or after a
// long outage) so the chart and correction-resolution have data immediately instead of
// repopulating one live point per 5-min poll. Points already within 4min of a stored reading
// are skipped, so it's idempotent and safe to merge. Returns true if it added anything.
async function backfillFromGraph(data, patientId) {
  try {
    const gr = await lluFetch(`/llu/connections/${patientId}/graph`);
    const graph = gr && gr.data && Array.isArray(gr.data.graphData) ? gr.data.graphData : [];
    if (!graph.length) return false;
    const times = data.glucoseHistory.map(g => g.time);
    const near = t => times.some(e => Math.abs(e - t) < 4 * 60000);
    let added = false;
    for (const gp of graph) {
      const gt = parseLibreTimestamp(gp.FactoryTimestamp || gp.Timestamp);
      if (!gt || isNaN(gt) || near(gt)) continue;
      const gmg = gp.ValueInMgPerDl != null ? gp.ValueInMgPerDl : (gp.Value != null ? gp.Value * 18.0182 : null);
      if (gmg == null) continue;
      // trend is left null on backfilled points - it's derived from adjacent live readings
      // (computeTrend) and none of the history consumers (chart, correction resolution) need it.
      data.glucoseHistory.push({ time: gt, value: parseFloat((gmg / 18.0182).toFixed(1)), trend: null });
      times.push(gt); // so later graph points dedupe against ones we just added
      added = true;
    }
    if (added) data.glucoseHistory.sort((a, b) => a.time - b.time);
    return added;
  } catch (e) { console.error('❌ Graph backfill error:', e.message); return false; }
}
...
async function fetchGlucose() {
  ...
  try {
    ...
    // FactoryTimestamp is UTC (preferred over Timestamp, the account's un-marked local time).
    // parseLibreTimestamp forces UTC interpretation so ts is a correct epoch on any timezone -
    // see the note on that function for why re-parsing the string on the client was the bug.
    const timestamp = m.FactoryTimestamp || m.Timestamp;
    const ts = parseLibreTimestamp(timestamp) || Date.now();

    const data = await loadData();
    let historyChanged = false;
    // Seed history from the graph endpoint only when the recent window is sparse - a cold start
    // or a gap. Steady-state polling (which already has dense recent history) never triggers the
    // extra call, so this adds no ongoing API load beyond the every-5-min poll.
    const sixHoursAgo = Date.now() - 6 * 3600000;
    if (data.glucoseHistory.filter(g => g.time > sixHoursAgo).length < 3 && cr.data[0].patientId) {
      historyChanged = await backfillFromGraph(data, cr.data[0].patientId);
    }
    ...
    // readingMs is the correct UTC epoch of the reading - the client/dosingContext use it for
    // age instead of re-parsing `timestamp` (the raw un-zoned string) in their own timezone.
    glucoseCache = { value: mmol, valueMgDl: mgdl, unit: 'mmol/L', trend: trend.arrow, trendLabel: trend.label, delta, deltaMinutes: delta != null ? deltaMinutes : null, timestamp, readingMs: ts, fetchedAt: Date.now() };
    ...
    // Persist if the live reading was new OR the graph backfill added points. 14 days, not 7 -
    // checkInsulinHealth()'s periodStats() compares two rolling 7-day windows (this week vs. the
    // prior week), so the prior window needs its own 7 days of glucose behind the current one.
    // Retaining only 7 days made that "prior week" comparison silently always-empty (readingCount
    // was always 0, so the note never fired). Pattern analysis in analysePatterns() still
    // explicitly scopes itself to the trailing 7 days regardless, so its "over the last 7 days"
