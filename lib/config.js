...
// A correction dosed alongside carbs can still teach a factor by netting the carbs out:
//   drop = units*factor - carbs*(factor/carbRatio)  =>  factor = drop / (units - carbs/carbRatio)
// The denominator ("effective correction units") must be comfortably above zero or the division
// blows up exactly like a too-low factor does - a dose that was almost entirely carb coverage
// carries essentially no correction signal, so below this we exclude it instead of guessing.
const MIN_EFFECTIVE_CORRECTION_UNITS = 1.0;
...
module.exports = {
  ...
  DEFAULT_SETTINGS, MIN_PLAUSIBLE_FACTOR, MAX_SUGGESTED_UNITS, MIN_EFFECTIVE_CORRECTION_UNITS,
